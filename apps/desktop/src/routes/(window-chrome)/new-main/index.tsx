...
import { generalSettingsStore, authStore } from "~/store";
...
export default function () {
	...
	createEventListener(window, "focus", () => {
		// Always use new recording flow for Klip - removed the setting check
...
function Page() {
	...
	// Klip-specific: Track auth sessions
	onMount(async () => {
	...
	createEffect(() => {
		console.log("targetMode changed:", rawOptions.targetMode);
		if (rawOptions.targetMode) {
			console.log("Opening target select overlays");
			commands.openTargetSelectOverlays();
		} else {
			console.log("Closing target select overlays");
